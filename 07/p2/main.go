...
func runPipeline(phases []int) int {
	...
	machines = append(machines, computer{output: make(chan int, 2)})

	for i := 1; i < len(phases); i++ {
		c := computer{input: machines[i-1].output, output: make(chan int, 2), result: make(chan int, 2)}
		c.start(phases[i])
		machines = append(machines, c)
	}

	machines[0].input = machines[len(phases)-1].output
	machines[0].start(phases[0])

	machines[0].input <- 0
	return <-machines[len(phases)-1].result
}
...
func (c *computer) start(phase int) {
	...
	copy(state, base)
	go runComputer(state, phase, c.input, c.output, c.result)
}

func runComputer(state []int, phase int, input chan int, output chan int, result chan int) {
	var o int
