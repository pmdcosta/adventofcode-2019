...
func handleOp(state []int, cursor int, in int) (int, bool) {
	...
	case 99:
		return 0, true
	case 1:
		return sum(state, cursor), false
	case 2:
		return mul(state, cursor), false
	case 3:
		return input(state, cursor, in), false
	case 4:
		return output(state, cursor), false
	}
...
func output(state []int, cursor int) int {
	fmt.Println(state[state[cursor+1]])
	return 2
}
