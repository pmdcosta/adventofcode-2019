...
func handleOp(state []int, cursor int, in int) (int, bool) {
	op := state[cursor] % 100
	switch op {
	case 99:
		return 0, true
	...
	case 4:
		return cursor + output(state, cursor), false
	case 5:
...
func input(state []int, cursor int, input int) int {
	if state[cursor]/100%10 == 0 {
...
func output(state []int, cursor int) int {
	var arg1 int
	if state[cursor]/100%10 == 0 {
		arg1 = state[state[cursor+1]]
	} else {
