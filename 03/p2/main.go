...
func parseInput(wire []string) map[point]int {
	var points = make(map[point]int)

	var current point
	var steps int
	for _, s := range wire {
		stepsD, _ := strconv.Atoi(s[1:])
		switch string(s[0]) {
		case "R":
			for i := 1; i <= stepsD; i++ {
				steps++
				current = point{current.x + 1, current.y}
				if s, found := points[current]; !found || (found && s > steps) {
...
func findInterceptions(wire1 map[point]int, wire2 map[point]int) map[point]int {
	var interceptions = make(map[point]int)

	for p, d1 := range wire1 {
		if d2, found := wire2[p]; found {
			steps := d1 + d2
			if s, found := interceptions[p]; !found || (found && s > steps) {
				interceptions[p] = steps
			}
		}
	}
...
func findClosest(points map[point]int) (current point, distance float64) {
	for p, d := range points {
		if float64(d) < distance || distance == 0 {
			current = p
			distance = float64(d)
		}
	}
