...
func parseInput(wire []string) map[point]bool {
...
func findInterceptions(wire1 map[point]bool, wire2 map[point]bool) (interceptions []point) {
	for p := range wire1 {
		if _, found := wire2[p]; found {
			interceptions = append(interceptions, p)
		}
	}
	return
}

func findClosest(points []point) (current point, distance float64) {
	for _, p := range points {
		d := math.Abs(float64(p.x)) + math.Abs(float64(p.y))
...
func printWire(wire1 map[point]bool, wire2 map[point]bool) {
	for y := 8; y >= -1; y-- {
		for x := -1; x <= 10; x++ {
			var w1, w2 bool
			if _, ok := wire1[point{x, y}]; ok {
				w1 = true
			}
			if _, ok := wire2[point{x, y}]; ok {
				w2 = true
			}
